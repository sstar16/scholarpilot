...
ScholarPilot：面向科研人员或公司技术部的全领域情报检索平台。从多个学术或专利数据源并行抓取文献和专利，AI生成中文摘要，用户反馈驱动画像学习，结束后自动转入每日监控模式。
...
### Scoring Agent System（2026-04-02，ai-powers 分支）

用 LLM Agent 替代硬编码评分公式，逐篇深度评估文献与用户研究目标的相关性。

**Feature Flag**: `enable_scoring_agent: bool = True`（`.env` 中 `ENABLE_SCORING_AGENT=false` 可关闭）

**核心组件**:

| 组件 | 文件 | 说明 |
|------|------|------|
| Scoring Agent | `harness/scoring_agent.py` | 逐篇 LLM 评分 0-10 + 一句话总结，asyncio.gather 并行 |
| Scoring Prompt | `harness/prompts/scoring.py` | 评分 rubric + system/user prompt 分离 |
| Memory Agent | `harness/memory_agent.py` | 4-bucket 反馈驱动的 LLM 记忆更新 |
| Memory Prompt | `harness/prompts/memory_update.py` | 结构化记忆生成 prompt |
| Deep Dive Agent | `harness/deep_dive_agent.py` | PDF 下载 + 全文提取 + LLM 深度分析 |
| Deep Dive Prompt | `harness/prompts/deep_dive.py` | 深度分析 prompt |
| Deep Dive Task | `workers/deep_dive_tasks.py` | Celery 异步任务 |
| CutoffSlider | `frontend/.../CutoffSlider.vue` | 斩杀线滑条 UI |

**评分流程**:
```
检索完成 → 传统评分（fallback 用）→ ScoringAgent.score_all（逐篇并行 LLM 评分）
  → 标记 above/below cutoff → 保存到 round_documents（agent_score, rationale, one_line_summary）
  → 前端 CutoffSlider 实时过滤 → 用户反馈 → MemoryAgent 更新记忆 → 下轮评分更准
```

**斩杀线（Cutoff）**:
- 默认 7.0，用户可通过 CutoffSlider 实时调整（0-10，步长 0.5）
- 服务端持久化到 `project.search_config["scoring_cutoff"]`
- 低于斩杀线的文档仍保存（`below_cutoff=True`），前端默认隐藏，可展开查看

**Memory Agent**:
- 与现有 `profile_service.update_profile_from_feedbacks()` 并行运行，互为冗余
- 记忆存储：`user_profiles.memory_text`（结构化 markdown）+ Redis 缓存
- 记忆注入：下轮评分时 Scoring Agent 的 prompt 包含 user_memory

**Deep Dive**:
- 用户触发：高分（≥7.0）或"非常相关"文档显示"深度阅读"按钮
- API: `POST /api/projects/{pid}/documents/{did}/deep-dive`（Celery 异步）
- PDF 来源：doc.pdf_url → Unpaywall 开放获取 → fallback 到仅摘要分析
- 文本提取：PyMuPDF 优先，fallback pdfplumber

**新增 API**:
- `POST /api/projects/{pid}/documents/{did}/deep-dive` — 触发深度分析
- `GET /api/projects/{pid}/documents/{did}/deep-dive` — 获取分析结果
- `PATCH /api/projects/{pid}/scoring-config` — 更新斩杀线等配置

**DB Migration 0005**:
- `round_documents`: +agent_score, +agent_rationale, +one_line_summary, +below_cutoff
- `documents`: +one_line_summary
- `user_profiles`: +memory_text, +memory_version
...
| 1.7 Scoring Agent | LLM 逐篇评分 + 斩杀线 + Memory Agent + Deep Dive | ✅ **ai-powers 分支**（2026-04-02）|
| 1.8 Open-Loop Buckets | 4桶分类系统 + 开放式轮次 + 监控解耦 | ✅ **ai-powers 分支**（2026-04-03）|
| 2 | 万方/百度学术、PDF全文、pgvector embedding、邮件通知 | 待开发 |
| 3 | EPO/WIPO专利、多语言检索、文献关系图、协作功能 | 规划中 |

### Phase 1.8: Open-Loop Bucket Architecture（2026-04-03）

**核心变化**：从固定5轮自动推进改为用户主导的开放式模式。

**新模型 `DocumentClassification`**（`models/document_classification.py`）:
- 4桶分类：very_relevant / relevant / uncertain / irrelevant
- 项目级持久化：唯一键 `(user_id, document_id, project_id)`，跨轮次保留
- 替代按轮次的 Feedback 模型（Feedback 表保留但不再自动推进轮次）

**新 API**:
- `PUT /api/projects/{pid}/documents/{did}/classify` — 分类/重分类到桶
- `GET /api/projects/{pid}/buckets` — 获取4桶内容
- `PUT /api/projects/{pid}/documents/{did}/move` — 桶间移动
- `POST /api/projects/{pid}/rounds/{rid}/finalize` — 手动结束轮次（触发 Memory + Profile 更新）
- `POST /api/projects/{pid}/monitoring/enable|disable` — 用户主动开关监控
- `GET|PATCH /api/projects/{pid}/monitoring` — 获取/更新监控配置

**前端新组件**:
- `BucketSidebar.vue` — 替代 RoundTimeline，左侧4桶可折叠列表+拖拽
- `BucketDropZone.vue` — 替代反馈按钮，点击分类+飞入动画
- `RoundHistory.vue` — 动态轮次历史列表
- `MonitoringPanel.vue` — 监控开关+配置面板
- `useFlyTo.ts` — 文档卡片飞入桶的 CSS 动画

**DB Migration 0006**: `document_classifications` 表 + 从 feedback 迁移数据 + max_rounds=0

**关键改动**:
- `progressive_search.create_next_round()` 移除 max_rounds 守卫
- `feedback.py` 移除自动创建下一轮 + 自动激活监控逻辑
- `activate_monitoring()` 接受显式配置参数，支持用户随时开关
- `ScoringAgent` 接受 `bucket_examples` 参数注入正例上下文
