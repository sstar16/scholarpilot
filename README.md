...
<img src="docs/screenshots/hero-landing.png" alt="ScholarPilot · 让科研人员把时间花在思考上" width="100%"/>

# ScholarPilot

**全领域科研情报检索平台 · 让科研人员把时间花在「思考」上**

多源并行检索 · AI 逐篇评分摘要 · 双层 Markdown 记忆 · 协作研究 · 知识图谱

[![Live Demo](https://img.shields.io/badge/🌐%20Live%20Demo-scholarpilot.top-1f6feb?style=for-the-badge)](https://scholarpilot.top/)
[![License: MIT](https://img.shields.io/badge/license-MIT-blue.svg?style=for-the-badge)](./LICENSE)
[![Tag](https://img.shields.io/badge/tag-main0-green.svg?style=for-the-badge)](https://github.com/sstar16/scholarpilot/releases/tag/main0)
[![Tauri 2](https://img.shields.io/badge/Tauri-2-FFC131?style=for-the-badge&logo=tauri)](https://tauri.app/)
[![Vue 3](https://img.shields.io/badge/Vue-3-4FC08D?style=for-the-badge&logo=vue.js)](https://vuejs.org/)

[🌐 在线体验](https://scholarpilot.top/) · [🖥️ 桌面客户端](#2-desktop-client推荐byok-自洽) · [📐 架构文档](./docs/architecture/) · [📜 LICENSE](./LICENSE)
...
ScholarPilot 帮你把"翻文献"的时间还给"想问题"。

你在对话里**自然描述研究方向**，系统就会：

1. **AI 自动规划检索关键词**（per-source 适配各数据源 boolean 语法）
2. **14+ 数据源并行抓取**（学术 + 全球专利 + 中国专利）
3. **LLM 逐篇 0-10 分评分** + 中文一句话摘要 + 关联理由
4. **斩杀线过滤** → 你只看高相关结果
5. **4 桶反馈**（非常相关 / 相关 / 不确定 / 不相关）→ 双层 Markdown 记忆学习 → 下一轮更精准
6. 文献库非空后可进入**协作研究模式** —— AI 边读边查边引用
7. 高分文献一键**深度阅读**（PDF 全文提取 + LLM 深度解读）
8. **知识图谱**自动抽实体/关系，cytoscape 力导向可视化

> **🌐 在线体验**：https://scholarpilot.top/ — Web 版，注册后即可建项目跑检索。
>
> **🖥️ 桌面端**：[client/](./client/) Tauri 2，BYOK 100%，所有 LLM 调用本地完成；产物在 `client/src-tauri/target/release/bundle/{msi,dmg,deb,appimage}/`。

<div align="center">

<img src="docs/screenshots/app-chat.png" alt="ScholarPilot 客户端 · 对话式项目管理" width="80%"/>

<sub>桌面客户端 · 项目「Transformer 推理加速」对话式建项目 + 关键词方案确认 + 4 桶反馈分类</sub>
...
### 🎯 真正"对话主体化"，不是套个 ChatGPT 壳子

每个流程（建项目 / 检索 / 关键词确认 / 4 桶分类 / 协作研究 / 深度阅读）都嵌入对话里作为**富消息气泡**，自然语言驱动。气泡本身是可交互 UI（KeywordConfirmPanel / BucketCardView / SourcePlanCard 等），不是聊天框输出文字让用户照做。状态机管理 7 种对话场景，AI 推断时机自动转入/退出流程，无需用户记 slash command。

### 🧠 双层 Markdown 记忆 — 学 Claude Code 的可解释记忆

**用户级 MEMORY.md**（身份/职业/研究大方向）+ **项目级 MEMORY.md**（本项目研究方向/子问题）。一个索引文件 + 多个详情 .md，AI 可读可写、用户可见可编辑。比向量 embedding RAG 更适合"研究方向画像"——可解释、可手编、迭代代价低，防跨项目污染。

### 🤖 Agent-First 检索 + Per-Source 关键词适配

QueryPlanAgent 自动规划三层降级关键词：复杂 boolean `(A OR B) AND (C OR D)` → 中等 AND → 简单兜底。每个源独立适配 API 语法（OpenAlex 5 词、arXiv 2-3 词、DBLP 2 词前缀、OpenAlex_zh 双格式 `|||`、EPO CQL）。用户可在确认气泡里编辑后再执行。

### 📊 知识图谱 — graphAgent + cytoscape.js 可视化

LLM 抽实体/关系（document / author / concept / topic / journal），cytoscape-fcose 力导向布局 500+ 节点流畅渲染，社区检测 + hubs/gaps 分析。学术综述场景天然契合。

### 💸 100% BYOK — 平台不背 LLM 成本

桌面客户端把 LLM Key 写 OS Keychain，所有评分/摘要/协作研究在本地完成，一行后端代码不改。Web 端走平台 key，桌面端 0 成本接入用户自己的 Anthropic / OpenAI / DeepSeek / Moonshot / jiekou.ai 中转 / Ollama 本地。

### 🌍 14+ 学术 + 专利数据源并行

学术：OpenAlex（2.5亿+）/ arXiv / EuropePMC / Crossref / DBLP / PubMed / ClinicalTrials / OpenAlex_zh
专利：EPO OPS（欧洲）/ Lens.org（90+ 国 1.6亿+）/ PatentHub（中国，三段付费 + PDF）

`asyncio.gather` 并行抓 → LLM 评分 → chord 摘要分发，单轮 30-60 秒拿到结果。付费源（PatentHub）走预算守门，超额二次确认可越权。

### 📦 桌面端 + Web 双形态共享后端

| 形态 | 适合 | 特点 |
|---|---|---|
| **🌐 Web** [scholarpilot.top](https://scholarpilot.top/) | 体验 / 临时使用 | 注册即用，平台托管，Cloudflare Tunnel 零暴露 |
| **🖥️ Desktop** Tauri 2 | 长期重度使用 | OS Keychain 存 token，本地 SQLite，BYOK 自付成本，原生 PDF 抓取绕反爬 |
...
### 0. 不装也能体验 → [scholarpilot.top](https://scholarpilot.top/)

打开 https://scholarpilot.top/ 注册（需要邀请码 — 在 [Issues](https://github.com/sstar16/scholarpilot/issues) 留言索取，或自部署），进首页 Journal 版 → 自然语言描述研究方向 → 走完检索 → 在 4 桶里反馈 → 看下一轮变化。Web 版用平台 LLM key，**0 配置即用**。
