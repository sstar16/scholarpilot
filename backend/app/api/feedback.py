...
from app.schemas.feedback import FeedbackSubmit, FeedbackResponse, MemoryUpdateOut, MemoryFileOut
...
async def submit_feedback(
    ...
    # P0.5：取返回值 → 拼 MemoryUpdateOut 给客户端写本地多文件
    memory_update_out: Optional[MemoryUpdateOut] = None
    ...
    if feedback_dicts and _cfg.enable_scoring_agent:
        try:
            ...
            if mu:
                memory_update_out = MemoryUpdateOut(
                    version=mu.version,
                    index_md=mu.index_md,
                    files=[MemoryFileOut(**f) for f in mu.files],
                    focus=mu.focus,
                )
        except Exception as e:
    ...
    return FeedbackResponse(
        ...
        memory_update=memory_update_out,
    )
