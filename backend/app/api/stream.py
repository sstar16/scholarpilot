...
async def stream_session_events(
    ...
    """
    SSE endpoint: streams workbench events for a conversation session.

    Event types (from workbench_hook.py):
    - tool_call_start/tool_call_end: {tool_name, call_id, args, status, duration_ms, result_preview, agent_name}
    - llm_call_start/llm_call_end: {call_id, provider, model, usage, cost_usd, latency_ms, finish_reason, text_preview}
    - llm_usage_delta: {usage, cost_usd} (cumulative)
    - agent_phase: {agent_name, from_phase, to_phase, description}
    - agent_message_delta: {message_id, delta, done, agent_name}
    """
    async def event_generator():
        yield f"event: connected\ndata: {json.dumps({'session_id': session_id})}\n\n"
        try:
            async for event in EventBus.subscribe_session(session_id):
                if await request.is_disconnected():
                    break
                event_type = event.get("event", "message")
                data = json.dumps(event.get("data", {}), ensure_ascii=False, default=str)
            ...
            logger.warning("[SSE] Session stream error %s: %s", session_id, e)
            yield f"event: error\ndata: {json.dumps({'error': str(e)})}\n\n"
