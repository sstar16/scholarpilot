"""
飞书（Lark）机器人通知 —— 推送用户反馈到飞书群。

国内直连飞书 open-apis，不需要代理。作为 Telegram 的备份通道（双发）。

配置：
- FEISHU_WEBHOOK_URL：飞书群机器人的 webhook 完整 URL
  （形如 https://open.feishu.cn/open-apis/bot/v2/hook/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxx）
- 未配置则静默跳过
"""
