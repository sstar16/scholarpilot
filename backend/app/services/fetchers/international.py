...
class OpenAlexZhFetcher(OpenAlexFetcher):
    ...
    中文搜索词数量 ≤3 个，否则 0 结果。
    """
    DEFAULT_TIMEOUT = 25.0  # 双策略并行，给更多时间
...
class SemanticScholarFetcher(AbstractFetcher):
    ...
    def _get_api_key(self) -> str:
        """优先 runtime 覆盖，回退 env。"""
        return os.getenv("SEMANTIC_SCHOLAR_API_KEY", "")

    async def fetch(self, query: str, max_results=20, year_from=None, year_to=None, language=None) -> List[Dict]:
        """使用 /paper/search/bulk（官方推荐，限流更宽松）+ API Key。"""
        papers = []
        params: dict = {
            "query": query,
            ...
            try:
                for attempt in range(3):
                    ...
                    if r.status_code == 200:
                        ...
                        logger.warning("[SemanticScholar] 429 rate limit, retry %d/3 after %ds", attempt + 1, delay)
                        await asyncio.sleep(delay)
                    else:
                        logger.error("[SemanticScholar] HTTP %d: %s", r.status_code, r.text[:200])
                        break
            except Exception as e:
...
from app.services.fetchers.bigquery_patents import BigQueryPatentsFetcher
...
# Mapping[SourceId, ...] 让 mypy 拒绝拼错的 key（如 "openalexx"）；模块加载尾部
# 调用 assert_registry_exhaustive 在 import 期捕获漏注册 / 多注册。
ALL_FETCHERS: Dict[SourceId, AbstractFetcher] = {
    ...
    "epo_ops": EPOFetcher(),
    "bigquery_patents": BigQueryPatentsFetcher(),
...
try:
    ...
    _local_kb = LocalKBFetcher()
    if _local_kb.is_available():
        ALL_FETCHERS["local_kb"] = _local_kb
except Exception:
    pass  # KB 模块未安装或数据不存在，静默跳过


# 注册期完备性检查 —— 漏注册或多注册立刻 RuntimeError，不到运行期才炸。
assert_registry_exhaustive(ALL_FETCHERS)


# ── Paid PDF source helpers（2026-05-03）─────────────────────────────────
# 把 "patenthub" 这类付费 PDF 源的判断从调用方挪到 fetcher metadata：
#   classification.py / fulltext_tasks.py / fulltext_service.py 全部用这里的
#   helper，不再硬编码字符串。
# 加新付费源时只需要在 fetcher 子类设 ``PAID_PDF = True`` + 实现
# ``download_pdf_for_doc``，调用方零改动。

def get_paid_pdf_source_ids() -> List[str]:
    """返回所有 PAID_PDF=True 的 source_id。"""
    return [sid for sid, f in ALL_FETCHERS.items() if getattr(f, "PAID_PDF", False)]


def is_paid_pdf_source(source_id: Optional[str]) -> bool:
    """判断 source 是否付费 PDF 源。None / 未注册 source 返回 False。"""
    if not source_id:
        return False
