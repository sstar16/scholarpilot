...
REQUIRES_CONFIG = {
    ...
    "soopat":       ["SOOPAT_EMAIL", "SOOPAT_PASSWORD", "SOOPAT_COOKIES"],
}
...
def _check_config(source_id: str) -> tuple[bool, str]:
    ...
    if not keys:
        return True, ""
    # soopat 特殊：有邮件密码 OR 有 cookie 都算齐全
    if source_id == "soopat":
        has_login  = bool(os.getenv("SOOPAT_EMAIL") and os.getenv("SOOPAT_PASSWORD"))
        has_cookie = bool(os.getenv("SOOPAT_COOKIES", "").strip())
        if has_login or has_cookie:
            return True, ""
        return False, "SOOPAT_EMAIL+PASSWORD 或 SOOPAT_COOKIES 均未配置"
