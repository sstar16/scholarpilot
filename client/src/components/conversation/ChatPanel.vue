  ...
  <div class="chat-panel" ref="panelRef">
    <div class="chat-panel__messages" ref="scrollRef">
      ...
      <div v-if="messages.length === 0" class="chat-panel__welcome">
        ...
        <p>用自然语言描述您的研究需求</p>
        <p class="sub">点击下方建议直接发送，或在输入框自由描述</p>
        <SuggestionChips
          :chips="welcomeChips"
          variant="welcome"
          @select="onChipSelect"
      ...
      <template v-for="(msg, i) in messages" :key="messageKey(msg, i)">
        ...
        <div v-if="msg.role === 'assistant'" class="chat-meta-line-wrapper">
          ...
          />
          <MetaLine
            v-else-if="metaTokens(msg)"
            :timestamp="msg.timestamp"
            :model="metaString(msg, 'model')"
            :input-tokens="metaNumber(msg, 'input_tokens') || 0"
            :output-tokens="metaNumber(msg, 'output_tokens') || 0"
            :elapsed-ms="metaNumber(msg, 'elapsed_ms') ?? null"
          />
...
// metadata 是 Record<string, unknown>；这些 helper 把 unknown 兜底成模板能用的具体类型
function metaTokens(m: { metadata?: Record<string, unknown> }): boolean {
  return !!(m.metadata && (m.metadata as { tokens?: unknown }).tokens)
}
function metaString(m: { metadata?: Record<string, unknown> }, key: string): string | undefined {
  const v = m.metadata?.[key]
  return typeof v === 'string' ? v : undefined
}
function metaNumber(m: { metadata?: Record<string, unknown> }, key: string): number | null {
  const v = m.metadata?.[key]
...
const completedScoringJobIds = computed(() => {
  ...
  for (const m of messages.value) {
    if (m.rich_type === 'pdf_import_final_card' && m.rich_data?.job_id) {
      ids.add(String(m.rich_data.job_id))
    }
  }
  return ids
})
...
// 动态引导气泡（按 CLAUDE.md 3 场景切换）
// 场景 1：项目刚创建 / 无 project（current_round === 0）
// 场景 2：有项目有轮次但文献库空（current_round > 0 && !hasNonEmptyLibrary）
// 场景 3：有文献库（hasNonEmptyLibrary）
const welcomeChips = computed<readonly string[]>(() => {
  const roundCount = props.projectId
    ? (projectStore.current?.current_round ?? 0)
    : 0
  const hasLibrary = bucketStore.total > 0

  if (roundCount === 0) {
    // 场景 1：引导用户开启第一轮检索
    return [
      '我想研究 Transformer 推理加速',
      '我想研究全固态锂电池',
      '推荐一个研究方向给我',
    ]
  }
  if (!hasLibrary) {
    // 场景 2：有项目但文献库尚空，引导开新检索或配置推送
    return [
      '开启新一轮检索',
      '配置每日推送',
      '导入一份 PDF',
...
async function onComposerSend(text: string) {
  ...
  if (isSearchInProgress.value) {
    toast.warning('检索进行中，请等待本轮结束后再发送消息')
    return
  }
  try {
...
async function onStartNextRound(payload?: { mode?: string }) {
  ...
  if (mode) {
    ...
    try {
      ...
      toast.error(e?.response?.data?.detail || '切换检索模式失败')
      return
    }
  }
  await startNewRound()
...
async function handlePendingRoundGate(err: any, onAfterFinalize: () => Promise<any>): Promise<boolean> {
  ...
  try {
    ...
    toast.success('喵~上一轮已收尾，这就帮你开新一轮！')
    await onAfterFinalize()
  } catch (finErr: any) {
    toast.error(finErr?.response?.data?.detail || '结束本轮失败')
  }
  return true
}
...
async function startNewRound() {
    ...
    try {
      ...
      if (prepareErr.response?.status === 400 && !isClarificationNeeded) {
        try {
          ...
          await projectStore.fetchProject(String(pid))
          await store.refreshMessages()
          if (round?.id) toast.success('检索已开始')
          return { ok: true }
        } catch (legacyErr) {
  ...
  if (!result.ok) {
    ...
    const detail = err?.response?.data?.detail
    toast.error(typeof detail === 'string' ? detail : '启动检索失败，请重试')
  }
...
async function onFinalizeRound() {
  ...
  try {
    ...
    toast.error(e?.response?.data?.detail || '结束失败')
  } finally {
...
async function onStartCollaboration(docIds: string[]) {
  ...
  try {
    ...
    toast.error(e?.response?.data?.detail || '进入协作失败')
  }
}
...
async function onFeatureTriggered(feature: string, result: any) {
  ...
  if (feature === 'new_round') {
    ...
    if (st === 'keyword_confirmation' || st === 'search_mode_selection') {
      ...
      if (!sid) { toast.warning('未找到会话'); return }
      try {
        await featuresApi.resetForNewRound(sid)
        await store.refreshMessages()
      } catch (e: any) {
        toast.error(e?.response?.data?.detail?.hint || e?.response?.data?.detail || '重置失败')
        return
      }
    }
...
let _unsubRoundBus: (() => void) | null = null
...
onMounted(() => {
  ...
  // pipeline 内部 message_appended 事件 → 立即刷新（不等 3s poll）
  _unsubRoundBus = getEventBus().subscribeChannel('round:', async (evt) => {
    if (evt.event !== 'message_appended') return
    const roundId = searchStore.currentRound?.id
    await Promise.all([
      store.refreshMessages(),
      roundId ? searchStore.refreshRoundStatus(roundId) : Promise.resolve(),
    ])
  })
})
...
onBeforeUnmount(() => {
  ...
  if (_unsubRoundBus) { _unsubRoundBus(); _unsubRoundBus = null }
})
</script>
