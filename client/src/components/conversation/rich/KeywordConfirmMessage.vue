<template>
  ...
  <div v-if="isDone" class="rich-msg rich-msg--keyword is-done rich-msg-enter">
  ...
  <div v-else class="rich-msg rich-msg--keyword rich-msg-enter" :key="`kw-active-${richData.round_id}`">
    ...
    <div class="rich-msg__body">
      <!-- store.keywordPlan 是内存态（刷新/切页后丢失），DB rich_data 是兜底来源 -->
...
const effectivePlan = computed(() => {
  const sp = searchStore.keywordPlan as Record<string, unknown> | null
  const curr = searchStore.currentRound
  const isSameRound = !!curr && curr.id === props.richData.round_id
  if (sp && isSameRound) {
...
/* Active state header (before confirmation) */
.rich-msg__header {
...
.rich-msg__body { padding: var(--space-3) var(--space-4); }
.loading-hint {
  display: flex;
  align-items: center;
  gap: var(--space-2);
