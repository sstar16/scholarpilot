    ...
    <div class="rich-msg__body" v-show="expanded">
      ...
      <div class="doc-list">
        <DocumentCard
          ...
          @upload-fulltext="(payload) => onUploadFulltext(String(doc.id), payload)"
...
import type { ClientBucket } from '@/data/sqlite/repos/bucketRepo'
...
async function onDocClassify(docId: string, bucket: string) {
  ...
  try {
    // C5：searchStore.classifyDocument 入参从 string 收紧到 ClientBucket
    await searchStore.classifyDocument(String(projectId.value), docId, bucket as ClientBucket)
    ...
    for (let i = 0; i < 60; i++) {
      ...
      try {
        ...
        if (updated[statusField] === 'available') {
          ...
          // M3.5: backend 下完后 fire-and-forget 拉本地副本到客户端 (per-user / per-project)
          if (format === 'pdf') {
            void downloadDocumentPdf(String(projectId.value), docId)
              .then((r) => {
                if (r.status === 'available') {
                  ElMessage.success(`✓ PDF 已存到本地（${((r.size ?? 0) / 1024).toFixed(1)} KB）`)
                }
              })
              .catch((err) => console.warn('[client] PDF local copy failed:', err))
          }
...
async function onUploadFulltext(
  ...
  // 0028: 上传时用户选同步策略
  // - "同步到云端" → backend 存 binary，多设备登录后能拉（source='uploaded_synced'）
  // - "仅本机保留" → 只写客户端 fs + 标 ownership，binary 不送 backend（source='uploaded_local'，最隐私）
  let syncToCloud: boolean
  try {
    const action = await ElMessageBox.confirm(
      `《${file.name}》将保存到该项目。\n\n` +
        `· 同步到云端：让你在其他设备登录后也能拉到这份\n` +
        `· 仅本机保留：更隐私（backend 拿不到 PDF），但只在本机能看`,
      `上传 ${format.toUpperCase()}`,
      {
        confirmButtonText: '同步到云端',
        cancelButtonText: '仅本机保留',
        distinguishCancelAndClose: true,
        type: 'info',
      },
    )
    syncToCloud = action === 'confirm'
  } catch (action) {
    // ElMessageBox 在 cancel 按钮 throw 字符串 'cancel'，X 按钮 throw 'close'
    if (action === 'cancel') {
      syncToCloud = false
    } else {
      return  // close / 其他 → 用户放弃上传
    }
  }
  ...
  try {
    if (syncToCloud) {
      ...
      // 仅本机：写客户端 fs，不打 backend
      const buf = await file.arrayBuffer()
      const { uploadDocumentPdf } = await import('@/data/sync/documentsSyncService')
      await uploadDocumentPdf(String(projectId.value), docId, new Uint8Array(buf))
    }

    // ownership marker —— 失败静默不阻塞用户体验
    try {
      const { markUploaded } = await import('@/data/sync/userDocumentsSyncService')
      await markUploaded(String(projectId.value), docId, format, syncToCloud)
    } catch (e) {
      console.warn('[markUploaded] failed (non-fatal):', e)
    }

    ElMessage.success(
      `✓ ${format.toUpperCase()} 上传成功${syncToCloud ? '（已同步云端）' : '（仅本机）'}`,
    )
    if (doc) {
    ...
    // 仅本机路径下 backend 没 binary，跳过 regenerate（LLM 摘要需要 backend 处理 PDF）
    // 这是用户接受的隐私 trade-off — 选了"仅本机"就放弃 AI 重摘要
    if (syncToCloud) {
      const roundId = searchStore.currentRound?.id
      if (roundId) await searchStore.loadRoundResults(roundId)
