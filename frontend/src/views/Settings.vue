...
const NAV_ITEMS: { id: Section; num: string; label: string; sub: string }[] = [
  { id: 'llm', num: '01', label: 'LLM · API 配置', sub: '模型来源与密钥' },
  { id: 'params', num: '02', label: '生成参数', sub: 'temperature / tokens · 本地偏好' },
  { id: 'about', num: '03', label: '关于', sub: '版本 · ISSN' },
]
...
async function loadProviders() {
  ...
  try {
    ...
    ElMessage.error(e.response?.data?.detail || 'Providers 加载失败')
  } finally {
    loading.value = false
  }
}
...
async function onTest(id: string) {
  ...
  try {
    ...
    ElMessage.success(`测试成功${preview ? ': ' + preview : ''}`)
  } catch (e: any) {
...
async function onSwitch(id: string) {
  try {
    ...
    activeProvider.value = id
    const p = providers.value.find((x) => x.provider_id === id)
    ElMessage.success(`已切换到 ${p?.display_name ?? id}`)
  } catch (e: any) {
