...
async def register(
    ...
    """注册：
    - X-Client-Type == "desktop"：开放注册，跳过邀请码（2026-05-08 客户端开放后）
    - 其它（web 等）：仍要求邀请码（向后兼容）
    - 首位用户自动 is_admin=True，让 admin 启动；后续可手动 SQL 改
    """
    client_type, client_version = _client_info(request)
    is_desktop = (client_type or "").lower() == "desktop"

    inv: InvitationCode | None = None
    if not is_desktop:
        # web 注册仍要邀请码
        code_str = (req.invitation_code or "").strip().lower()
        if not code_str:
        ...
        # desktop：邀请码可选（如果传了就当作 admin 给的，校验并消耗；否则跳过）
        code_str = (req.invitation_code or "").strip().lower()
        if code_str:
            inv_result = await db.execute(
                select(InvitationCode).where(InvitationCode.code == code_str)
            ...
            inv = inv_result.scalar_one_or_none()
            if inv is not None and (inv.is_used or inv.is_expired):
                # 客户端传了无效码：不阻塞注册，仅置空不消耗
                inv = None
