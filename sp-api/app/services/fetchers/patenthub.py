...
class PatentHubFetcher(AbstractFetcher):
    ...
    # ─────────────────────────────── PDF 下载（已下放客户端） ───────────────────
    # 历史：``download_pdf_file`` / ``download_pdf_by_patent`` 把 PDF 写到
    # ``outfile`` 本地路径。2026-05-08 sp-api 零本地 PDF 改造：fulltext_service
    # 的 ``stream_pdf_proxy`` 直接 ``httpx.stream`` PatentHub /api/pdf，chunked
    # 转发给客户端，不再落盘。这两个方法保留 stub 是为了：
    #   1) ``base.AbstractFetcher.download_pdf_for_doc`` 仍要求付费源实现，给个
    #      显式 NotImplementedError 让上游路由（理论上不该再调到）立刻爆。
    #   2) 兼容 ``test_fetchers.py`` 的 ``PAID_PDF=True`` 断言（接口形态没变）。
    async def download_pdf_for_doc(self, doc: Dict, outfile: Path) -> bool:  # type: ignore[override]
        raise NotImplementedError(
            "patenthub.download_pdf_for_doc 已废弃 — sp-api 不再服务端落盘 PDF；"
            "客户端走 POST /api/fulltext/proxy/patenthub/{external_id} 拿 stream"
        )
